...
There will be an executable file "acme_client.jar".

Usage: java -jar acme_client.jar --command <command> [options]
  Options:
    --account-key, -a
    ...
    --command
       ...
       generate-certificate,download-certificate, revoke-certificate, renew-certificate. Read below 'Main commands' section
       to know how to use these commands.
    --csr, -c
       CSR file.
    --dns-digests-dir
...
2) generate private domain key
openssl genrsa -out example.com.key 2048 

3) Configure openssl to use alternative names. I.e. if you have only one domain you should skip this section

3.1) Copy openssl.cnf config to your custom location. For example on CentOS 7:
cp /etc/pki/tls/openssl.cnf openssl.cnf

3.2) edit your openssl cnf file and change next parameters (you should create your own [alt_names]). Add only alternative name to your [alt_names], do not put main domain (i.e. example.com) into your alt_names:
[ req ]
...
4) generate csr (Certificate Signing Reques) based on private domain key
...
10) Done. You should see next files in /path/to/cert/ directory:
